...
class App extends Component {
  ...
  componentDidMount() {
    this.timerId = setInterval(this.setSecondsTimer, 1000)
  }
  ...
  setSecondsTimer = () => {
    ...
    if (time !== 0) {
      ...
      this.setState({isScoreCardDisplayed: true})
    }
  }
  ...
  onImageClick = thumbnailUrl => {
    ...
    const clickedImageValue = imagesList.filter(
      eachImage => eachImage.thumbnailUrl === thumbnailUrl,
    )
    const {imageUrl} = clickedImageValue[0]
    if (imageUrl === imgUrl) {
      const newImageUrlToDisplay =
        imagesList[Math.floor(Math.random() * imagesList.length)].imageUrl
      console.log(newImageUrlToDisplay)
      this.setState(prevState => ({
        score: prevState.score + 1,
        imgUrl: newImageUrlToDisplay,
      }))
    } else {
      clearInterval(this.timerId)
      this.setState({isScoreCardDisplayed: true})
  ...
  render() {
    const {isScoreCardDisplayed, category, imgUrl, score, time} = this.state
    ...
    return (
      <div className="main-container">
        ...
        <div className="content-div">
          {!isScoreCardDisplayed && (
            <div className="first-div">
              <img src={imgUrl} className="big-image" alt="match" />
              <ul className="tab-elements">
                {tabsList.map(eachImage => (
                  <li key={eachImage.id}>
                    <button
                      type="button"
                      className={`tab-button ${
                        category === eachImage.tabId ? 'highlight-text' : ''
                      }`}
                      onClick={() => this.onTabClick(eachImage.tabId)}
                    >
                      {eachImage.displayText}
                    </button>
                  </li>
                ))}
              </ul>
              <ul className="thumbnail-images">
                {thumbnailList.map(eachImage => (
                  <li key={eachImage.id}>
                    <button
                      type="button"
                      onClick={() => this.onImageClick(eachImage.thumbnailUrl)}
                    >
                      <img
